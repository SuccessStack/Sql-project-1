...
-- Revenue contribution by time of day and day of the week
SELECT 
    day_name,
    time_of_day,
    SUM(total) AS total_revenue
...
FROM sales
GROUP BY product_line
ORDER BY avg_quantity_sold DESC, avg_rating DESC;

-- High-value customers spending above a certain threshold (e.g., $500)
SELECT 
    customer_type,
...
-- Sales efficiency by branch
SELECT 
    branch,
    product_line,
    SUM(total) / COUNT(DISTINCT invoice_id) AS revenue_per_invoice
FROM sales
GROUP BY branch, product_line
...
SELECT 
    ...
    AVG(total) AS avg_spending_per_purchase
FROM sales
GROUP BY customer_type
HAVING purchase_frequency > 1
ORDER BY purchase_frequency DESC, avg_spending_per_purchase DESC;
